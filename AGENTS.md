...
lib/
  ...
  site.ts                         # SITE_URL/SITE_HOST — única fuente del dominio canónico (apex, derivado de NEXT_PUBLIC_BASE_URL), usado por metadataBase, robots.ts, sitemap.ts, llms.txt, JsonLd.tsx
...
- **Meta Pixel:** `components/MetaPixel.tsx`, `next/script` con `strategy="afterInteractive"` — por eso no aparece en el HTML servido por el servidor (`curl`), se inyecta client-side tras hidratar. Dispara `PageView` una vez al cargar (el script inline), y de nuevo en cada cambio de ruta o querystring client-side vía un `PageviewTracker` interno (`usePathname`+`useSearchParams`, envuelto en su propio `<Suspense>` para no optar la página fuera del render estático). `ViewContent`/`InitiateCheckout`/`Purchase` tipados en `lib/meta-pixel.ts` (guard sobre `window.fbq`, no lanza si el script no cargó). `InitiateCheckout` integrado en `lib/checkout-client.ts` → `startCheckout()`. `ViewContent` integrado vía `components/ui/ViewContentTracker.tsx` (client component sin marcado, un solo `useEffect`), montado en `app/resultado/[jobId]/page.tsx` (Server Component) solo en la rama de job completado. `Purchase` real sigue siendo CAPI server-side (ver abajo); el `trackPurchase()` de navegador es opcional y queda sin llamar a propósito (redundante con CAPI), comentado en `app/gracias/GraciasClient.tsx`. Verificado en vivo contra la API real de Meta: el token de `META_CAPI_TOKEN` sí tiene permiso de escribir eventos en el pixel de `NEXT_PUBLIC_FB_PIXEL_ID` (un `Purchase` de prueba enviado directo devolvió `events_received: 1`), y `fbevents.js`/el endpoint `facebook.com/tr` responden 200. `lib/meta-capi.ts` acepta `META_CAPI_TEST_EVENT_CODE` opcional (solo dev) para que un `Purchase` de CAPI aparezca en la pestaña "Eventos de prueba" de Events Manager en tiempo real — sin él, un evento CAPI es real pero solo aparece en el resumen normal de Eventos, con retraso. Un evento "Subscribe" que puede aparecer en Events Manager **no viene de este código** (grep del repo confirma que no existe ningún `fbq('track', 'Subscribe')` ni texto "suscríbete" en la UI) — es el Automatic Events / Event Setup Tool de Meta, configurado desde su panel, no desde el código.
...
- **Meta Pixel + CAPI server-side** ✅ hecho — `Purchase` se dispara desde `app/api/webhooks/stripe/route.ts` vía `lib/meta-capi.ts` al confirmar el pago (requiere `META_CAPI_TOKEN`, se omite en silencio si falta). `InitiateCheckout` se dispara del lado del cliente al iniciar el checkout. **Advanced Matching agregado tras un match quality de 3.2/10 con solo `em`:** `user_data` ahora manda también `external_id` (hash del `userId` interno) y, cuando están disponibles, `client_ip_address`/`client_user_agent`/`fbp`/`fbc` sin hashear — capturados en `POST /api/checkout` (única request que trae la IP/UA/cookies reales del comprador; el webhook de Stripe llega desde los servidores de Stripe, sin esos datos) y pasados al webhook vía `checkoutSession.metadata`. `ph`/`fn`/`ln` quedaron fuera a propósito: no se captura teléfono en Stripe Checkout hoy (requeriría `phone_number_collection`), y `User.name` no está separado en nombre/apellido ni existe para usuarios de magic link — decisión pendiente de negocio, no técnica. **Confirmado en producción** con una compra real: match quality mejoró tras agregar Advanced Matching. `sendPurchaseCapiEvent` loguea siempre (éxito y fallo, sin gate de `NODE_ENV`, así que aparece en los logs de Vercel) y devuelve `CapiSendResult` (`{ ok, status, body, error }`) en vez de `void` — el webhook no usa ese valor de retorno hoy, queda disponible para debugging futuro sin generar fricción de tipos.
...
**Si `ADMIN_PASSWORD` no está definida, `/admin` completo no existe** — 404, no un formulario de login que anuncie la herramienta. `/api/admin/*` sin cookie válida también responde 404 siempre (un fetch no tiene a dónde "redirigir" de forma útil). **`/admin/*` (páginas) es distinto:** si el panel está configurado pero la sesión falta o expiró, `proxy.ts` redirige a `/admin` (la página de login, ya pública) en vez de responder 404 — un 404 duro ahí se veía, y se reportó, como "la página no existe" en vez de "hay que volver a entrar". El 404 se reserva para cuando el panel entero no está configurado, que es el caso que de verdad no se quiere anunciar. Sesión de admin: **30 días** (antes 8 horas — demasiado corta para una herramienta que se revisa a diario, ver `lib/admin-auth.ts`).

**El chequeo de acceso vive en tres capas, no una sola, y las tres son necesarias:**
1. `proxy.ts` (`guardAdmin`) — la que de verdad determina el status code/comportamiento HTTP. Este proyecto tiene un `app/loading.tsx` raíz, lo que hace que **toda** ruta streamee un shell con `200` antes de que un `notFound()` más adentro del árbol de React pueda resolverse — confirmado probando `/resultado/<id-inexistente>`, que ya tenía este mismo comportamiento antes de que existiera el panel de admin. Un 404 real (o una redirección real) solo se puede garantizar deteniendo la petición antes de que empiece el streaming, así que `proxy.ts` valida la cookie con Web Crypto (`crypto.subtle`, no `node:crypto` — el runtime de Proxy/Middleware puede ser Edge) y responde 404 (panel no configurado) o redirige a `/admin` (sesión inválida/expirada) sin tocar React.
...
**Ajuste manual de créditos** (`lib/credits.ts` → `adjustCreditsByAdmin`): agregó el reason `ADMIN_ADJUSTMENT` al enum `CreditReason` y una columna `CreditTransaction.note` (texto libre, obligatorio solo para este reason) — es la única mutación de todo el panel, y como el resto de `lib/credits.ts`, nunca es un `update` suelto: un `updateMany` condicional (`credits >= -delta`, válido tanto para sumar como para restar) dentro de la misma transacción que crea el `CreditTransaction` impide dejar el saldo negativo y garantiza que todo ajuste deja rastro. El formulario (`AdjustCreditsForm.tsx`, client component) pide confirmación explícita antes de enviar — vía `components/admin/ConfirmModal.tsx` (con el estilo denso del panel de `components/admin/styles.ts`, no un `window.confirm()` nativo del navegador), con el motivo visible en el mensaje.

**Lista de `/admin/usuarios`** (`app/admin/(protected)/usuarios/page.tsx`) excluye siempre `isAnonymous: true` — los usuarios anónimos (cookie `uid` sin cuenta real, dispositivos que solo usaron el free tier) no tienen correo ni pueden comprar, y antes aparecían como filas `(sin correo / anónimo)` sin aportar nada útil al panel. `getTodayYesterdaySummary` en `lib/admin-metrics.ts` ya excluía anónimos de sus conteos desde antes — este era el único punto que le faltaba el filtro.
...
1. **Google Cloud Console** → en las mismas credenciales OAuth 2.0 ya creadas (tipo "Web application"), agregar a **Authorized redirect URIs** (además de la del túnel de ngrok, que se queda para seguir probando en local):
   - `https://revivelos.com/api/auth/callback/google`
...
|---|---|---|---|
| `NEXT_PUBLIC_BASE_URL` | túnel de ngrok | `https://revivelos.com` | Se usa para construir el `webhookUrl` que recibe fal — si apunta al túnel, los webhooks de producción nunca llegan |
| `AUTH_URL` | túnel de ngrok | `https://revivelos.com` | Determina el dominio de las cookies de sesión y las redirect URIs válidas de OAuth |
...
| `STRIPE_WEBHOOK_SECRET` | el que imprime `stripe listen` en dev | **el del endpoint configurado en el dashboard de Stripe** apuntando a `https://revivelos.com/api/webhooks/stripe` | Cada endpoint (dev vs. producción) tiene su propio secreto de firma — no son intercambiables |
...
2. **En Vercel → Settings → Domains**: confirmar `revivelos.com` (apex) como dominio de producción primario, y que `www.revivelos.com` siga con el redirect 308 existente hacia el apex.
...
4. **Google Cloud Console**: agregar `https://revivelos.com/api/auth/callback/google` a las redirect URIs autorizadas (ver sección de arriba).
...
8. **Después del primer deploy**: probar el flujo completo (`/crear` → restauración gratis → `/resultado`) contra `https://revivelos.com` real, no contra ngrok. Confirmar que el login con Google funciona con la redirect URI nueva.
